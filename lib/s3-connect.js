module.exports = function(connect) {
    ...
    S3Store.prototype.set = function(sid, sess, fn) {
        sid = _uri_encode(sid);
        fn = fn || function () {};
        this.db.get(sid, function (err, oldsess) {
            ...
            if (err) {
                ...
                oldsess = JSON.parse(oldsess);
                var accessGap = sess.lastAccess - oldsess.lastAccess;
                ...
                if (oldsess.cookie.expires) {
                    oldsess.cookie.expires = null;
                }
                sess.lastAccess = null;
                sess.cookie.originalMaxAge = null;
                oldsess.lastAccess = null;
                oldsess.cookie.originalMaxAge = null;
                // Compare new session to current session, save if different
                // or setThrottle elapses
                if (JSON.stringify(oldsess) !== JSON.stringify(sess)
